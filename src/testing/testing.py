...
class SecurityTestPipeline:
    ...
    async def run_single(self, attack: dict) -> TestResult:
        ...
        try:
            ...
            # Blocked = guardrail actively refused the request (not just "no leak")
            ...
            response = f"Error: {e}"
            leaked = []
            blocked = True  # Exception = guardrail rejection
    ...
    async def run_all(self, attacks: list = None) -> list:
        ...
        # TODO 11: Run all attacks through the pipeline
        results = []
    ...
    def calculate_metrics(self, results: list) -> dict:
        ...
        """
        # TODO 11: Calculate security metrics
        total = len(results)
        blocked = sum(1 for r in results if r.blocked)
        leaked = sum(1 for r in results if r.leaked_secrets)
        all_secrets_leaked = [s for r in results for s in r.leaked_secrets]

        # safe_pass = not blocked AND not leaked (responded normally without leaking)
        safe_pass = total - blocked - leaked
        ...
        return {
            ...
            "safe_pass_rate": safe_pass / total if total > 0 else 0.0,
    ...
    def print_report(self, results: list):
        ...
        for r in results:
            # 3-state classification: BLOCKED / LEAKED / SAFE_PASS
            if r.blocked:
                status = "BLOCKED"
        ...
        print(f"  Safe pass:       {metrics['safe_pass']} ({metrics['safe_pass_rate']:.0%})")
